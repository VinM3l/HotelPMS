...
export default function Dashboard({ currentDate, onAddBooking }) {
  ...
  const [roomModal, setRoomModal] = useState(null) // room number
  ...
  const filteredFloors = useMemo(() => {
    ...
    for (const [floor, nums] of Object.entries(floorMap)) {
      const filtered = nums.filter(num => {
        ...
        if (q) {
          return num.toLowerCase().includes(q) || b?.guest?.toLowerCase().includes(q)
        }
        return true
      })
    ...
    return result
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [floorMap, rooms, bookings, filter, search, todayStr])
              ...
              {nums.map(num => {
                ...
                return (
                  ...
                  >
                    {isOcc && (
                      <span
                        className={`absolute top-1.5 right-1.5 text-[11px] ${b?.key_deposit ? 'opacity-60' : 'opacity-100'}`}
                    ...
                    {s === 'maintenance' ? (
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-1 mt-1 flex-wrap">
                          ...
                          {!b.key_deposit && <span className="text-[9px] bg-white/20 px-1 rounded">No dep.</span>}
                        </div>
                      </>
                    ) : (
                      <>
