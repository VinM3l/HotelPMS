...
import {
  ...
  bookingTotalDue, bookingAmountPaid, calcNights, fmtDate
} from '../lib/utils'
      ...
      {filtered.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-white border border-gray-100 rounded-xl px-4 py-3">
            ...
            <div className={`text-xl font-extrabold ${totalOwed > 0 ? 'text-red-500' : 'text-brand'}`}>
            ...
            <div key={b.id} className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-3 hover:border-gray-200 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-gray-900 truncate flex items-center gap-2">
                  {b.guest}
                  {isCheckin  && <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-medium">Check-in today</span>}
                  {isCheckout && <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-medium">Check-out today</span>}
                </div>
              ...
              <div className="text-right flex-shrink-0">
                ...
                <div className={`text-xs font-medium ${
                  payStatus === 'full' ? 'text-brand' : payStatus === 'partial' ? 'text-amber-600' : 'text-red-500'
                }`}>
                  {payStatus === 'full' ? '✅ Paid' : payStatus === 'partial' ? `${peso(bal)} balance` : 'Unpaid'}
                </div>
              </div>
