...
export default function Dashboard({ currentDate, onAddBooking }) {
  ...
  const filteredFloors = useMemo(() => {
    ...
    for (const [floor, nums] of Object.entries(floorMap)) {
      const filtered = nums.filter((num) => {
        ...
        if (filter === 'no-deposit') {
          if (!isOcc || b?.key_deposit) return false
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
        ...
        {[
          {
            label: 'Occupancy',
            val: stats.pct + '%',
            sub: `${stats.occ} of ${stats.total} rooms`,
            color: '',
          },
          { label: 'Occupied', val: stats.occ, sub: '', color: 'text-blue-600' },
          { label: 'Vacant', val: stats.vac, sub: '', color: 'text-brand' },
          {
            label: 'No key deposit',
            val: stats.noDeposit,
            sub: `of ${stats.occ} occupied`,
            color: 'text-red-500',
          },
          {
            label: 'Outstanding',
            val: peso(stats.totalOwed),
            sub: 'balance owed',
            color: stats.totalOwed > 0 ? 'text-amber-600' : 'text-brand',
          },
        ].map((s) => (
          <div
            key={s.label}
      ...
      {isToday &&
        showBanner &&
        (todayActivity.checkIns.length > 0 || todayActivity.checkOuts.length > 0) && (
            ...
            <div className="flex items-center gap-3 flex-1 flex-wrap">
              {todayActivity.checkIns.length > 0 && (
                <span className="flex items-center gap-1.5 text-green-700 font-medium">
                  ✅ <strong>{todayActivity.checkIns.length}</strong> check-in
                  {todayActivity.checkIns.length !== 1 ? 's' : ''} today
                  <span className="text-green-500 text-xs">
                    ({todayActivity.checkIns.map((b) => `Rm ${b.room_number}`).join(', ')})
                  </span>
                </span>
              )}
              {todayActivity.checkOuts.length > 0 && (
                <span className="flex items-center gap-1.5 text-blue-700 font-medium">
                  🚪 <strong>{todayActivity.checkOuts.length}</strong> check-out
                  {todayActivity.checkOuts.length !== 1 ? 's' : ''} today
                  <span className="text-blue-500 text-xs">
                    ({todayActivity.checkOuts.map((b) => `Rm ${b.room_number}`).join(', ')})
                  </span>
                </span>
              )}
            </div>
        ...
        {Object.entries(filteredFloors).length === 0 ? (
          ...
          Object.entries(filteredFloors).map(([floor, nums]) => (
            <div key={floor} className="mb-4">
              <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 pl-1">
                {floor}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="absolute top-1.5 right-1.5 flex gap-0.5">
                        ...
                        {hasBalance && (
                          <span className="text-[11px]" title="Balance outstanding">
                            💰
                          </span>
                        )}
                      </div>
                      ...
                      <div className="text-[10px] opacity-70 mb-1.5 leading-tight">
                        {room?.label}
                      </div>
                      ...
                      {s === 'maintenance' ? (
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-1 mt-1 flex-wrap">
                            <span
                              className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${SRC_BADGE_CLASSES[b.source] || ''}`}
                            >
                              {srcShort(b.source)}
                            </span>
                            {isCheckout && (
                              <span className="text-[9px] bg-white/20 px-1 rounded">Out</span>
                            )}
                            {isCheckin && !isCheckout && (
                              <span className="text-[9px] bg-white/20 px-1 rounded">In</span>
                            )}
                          </div>
                        </>
                      ) : (
