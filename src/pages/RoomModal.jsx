...
export default function RoomModal({ roomNumber, currentDate, onClose, onAddBooking }) {
  ...
  const markCheckedIn = async (b) => {
    await updateBookingField(b.id, {
      ...
      checked_in_by: user?.label || user?.username || null,
    })
    await reload()
  ...
  const confirmMove = async () => {
    const b = moveBooking
    if (!b || !moveTarget || moveTarget === b.room_number) {
      toast('Select a different room')
      return
    }
    if (
      !confirm(
        `Move ${b.guest} from room ${b.room_number} to ${moveTarget}? This cannot be undone.`,
      )
    )
      return
    ...
    const newBooking = {
      ...
      room_moves: [
        ...(b.room_moves || []),
        { from: b.room_number, to: moveTarget, date: moveDate, note: moveNote },
      ],
      key_deposit: false,
      checked_out: false,
      invalid_checkout: false,
    ...
    <>
        ...
        footer={
          <>
            ...
            <button
              ...
              className="px-4 py-2 text-sm bg-brand text-white rounded-lg font-medium hover:bg-brand-dark"
            >
              + Add Booking
            </button>
          </>
          ...
          <div className="space-y-4">
            {currentBookings.length === 0 ? (
              ...
              currentBookings.map((b) => {
                ...
                const coActive =
                  b.checked_out && (b.checked_out_date ? todayStr >= b.checked_out_date : true)
                const invActive =
                  b.invalid_checkout &&
                  (b.invalid_checkout_date ? todayStr >= b.invalid_checkout_date : true)
                return (
                  <div key={b.id} className="border border-gray-100 rounded-xl p-4 bg-gray-50">
                    ...
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div>
                        ...
                        <div className="text-xs text-gray-500 mt-0.5">
                          {shortDate(b.checkin)} – {shortDate(b.checkout)} · {nights} night
                          {nights !== 1 ? 's' : ''}
                        </div>
                        {b.notes && (
                    ...
                    <div className="bg-white rounded-lg border border-gray-100 p-3 mb-3 text-sm">
                      <div className="flex justify-between text-gray-600 mb-1">
                        <span>Total due</span>
                        <span className="font-semibold">{peso(due)}</span>
                      </div>
                      <div className="flex justify-between text-green-700 mb-1">
                        <span>Paid</span>
                        <span className="font-semibold">{peso(paid)}</span>
                      </div>
                      {bal > 0 && (
                      ...
                      {addPaymentBid === b.id ? (
                        <div className="mt-2 pt-2 border-t border-gray-100 space-y-2">
                          ...
                          <div className="flex gap-2 justify-end">
                            <button
                              onClick={() => setAddPaymentBid(null)}
                              className="text-xs text-gray-500 hover:text-gray-700"
                            >
                              Cancel
                            </button>
                            <button
                              onClick={() => addPayment(b)}
                        ...
                        <button
                          onClick={() => {
                            setAddPaymentBid(b.id)
                            setPmtAmount(String(bal || due))
                            setPmtDate(fmtDate(currentDate))
                            setPmtNote('')
                          }}
                    ...
                    {coActive && b.checked_out_by && (
                      <div className="text-xs text-gray-400 mb-2">
                        🚪 Checked out by {b.checked_out_by}
                        {b.checkout_time ? ` · ${fmtTimestamp(b.checkout_time)}` : ''}
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      {b.checkin_time && (
                        <div className="text-xs text-gray-400 py-2 px-3 flex items-center gap-1">
                          ...
                          {b.checked_in_by ? ` · ${b.checked_in_by}` : ''}
                          <button
                            onClick={() => clearCheckinTime(b)}
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm font-semibold">
                {new Date(calYear, calMonth, 1).toLocaleDateString('en-PH', {
                  month: 'long',
                  year: 'numeric',
                })}
              </span>
            ...
            {(() => {
              ...
              for (let i = 0; i < firstDay; i++) {
                cells.push(
                  <div key={`p${i}`} className="text-gray-200 text-xs text-center py-1.5">
              ...
              for (let d = 1; d <= daysInMonth; d++) {
                ...
                const b = roomBookings.find((bk) =>
                  isInRangeInclusive(new Date(ds + 'T00:00:00'), bk.checkin, bk.checkout),
                )
                ...
                cells.push(
                  <div
                    key={d}
                    className={`text-xs text-center py-1 rounded-sm ${bg} ${isToday ? 'ring-2 ring-brand' : ''} relative`}
                  >
                    <div className="font-medium">{d}</div>
                    {b && (
                      <div className="text-[9px] truncate leading-none mt-0.5 opacity-70">
                        {b.guest.split(' ')[0]}
                      </div>
                    )}
                    {hasPmt && (
              ...
              for (let i = 1; i <= (rem ? 7 - rem : 0); i++) {
                cells.push(
                  <div key={`n${i}`} className="text-gray-200 text-xs text-center py-1.5">
                    {i}
                  </div>,
                )
              }
      ...
      {extendId && extendBooking && (
        <Modal
          ...
          title="⟳ Extend Stay"
          subtitle={`${extendBooking.guest} · Room ${roomNumber}`}
          maxWidth="max-w-sm"
          zIndex="z-[60]"
          footer={
            <>
              <button onClick={() => setExtendId(null)} className="px-4 py-2 text-sm text-gray-500">
                Cancel
              </button>
              <button
                onClick={confirmExtend}
                className="px-4 py-2 text-sm bg-amber-500 text-white rounded-lg font-medium hover:bg-amber-600"
              >
                Confirm
              </button>
            </>
          ...
          <div className="space-y-3">
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              ...
              <div className="text-lg font-extrabold text-amber-800">
                {shortDate(extendBooking.checkout)}
              </div>
            </div>
            <div>
      ...
      {moveId && moveBooking && (
        <Modal
          ...
          title="↔ Move Guest"
          subtitle={`${moveBooking.guest} currently in Room ${moveBooking.room_number}`}
          maxWidth="max-w-sm"
          zIndex="z-[60]"
          footer={
            <>
              <button onClick={() => setMoveId(null)} className="px-4 py-2 text-sm text-gray-500">
                Cancel
              </button>
              <button
                onClick={confirmMove}
          ...
          <div className="space-y-3">
            <div>
              ...
              >
                ...
                {sortRoomKeys(rooms.map((r) => r.number))
                  .filter((n) => n !== roomNumber)
                  .map((n) => (
                    <option key={n} value={n}>
                      Room {n} — {rooms.find((r) => r.number === n)?.label}
                    </option>
                  ))}
              </select>
            </div>
