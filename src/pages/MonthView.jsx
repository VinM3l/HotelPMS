    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-auto">
        <table
          ...
          style={{ minWidth: `${daysInMonth * 52 + 120}px` }}
        >
          <thead className="sticky top-0 z-10 bg-white">
            <tr>
              <th className="sticky left-0 z-20 bg-white border border-gray-200 px-2 py-1.5 text-left font-semibold text-gray-500 min-w-[110px]">
                Room
              </th>
                  ...
                  {days.map(({ d, ds, isToday }) => {
                    if (room?.status === 'maintenance') {
                      return (
                        <td
                          key={d}
                          className="border border-gray-200 bg-[#374151] p-0"
                          title="Maintenance"
                        >
                    ...
                    const booking = roomBookings.find((b) =>
                      isInRangeInclusive(parseDate(ds), b.checkin, b.checkout),
                    )
                    ...
                    return (
                      ...
                      >
                        <div className="w-full h-7 px-0.5 flex items-center overflow-hidden relative">
                          {isCI && (
                            <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-white/60 rounded-r" />
                          )}
                          {isCO && (
                            <div className="absolute right-0 top-0 bottom-0 w-0.5 bg-white/60 rounded-l" />
                          )}
                          {booking && (isCI || d === 1) && (
