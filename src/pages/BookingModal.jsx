...
export default function BookingModal({ bookingId, preRoom, currentDate, onClose, defaultHotelId }) {
  ...
  const [hotelId, setHotelId] = useState(
    existing?.hotel_id || defaultHotelId || rooms[0]?.hotel_id || 'square',
  )
  ...
  const save = async () => {
    ...
    if (conflict) {
      toast(`🚫 Room ${room} conflicts with ${conflict.guest}`)
      return
    }
      ...
      <div className="space-y-4">
              ...
              {sortRoomKeys(
                rooms
                  .filter(
                    (r) =>
                      r.hotel_id === hotelId &&
                      (r.status !== 'maintenance' || r.number === existing?.room_number),
                  )
                  .map((r) => r.number),
              ).map((n) => {
                const isConflict =
                ...
                return (
                  <option
                    key={n}
                    value={n}
                    style={isConflict || isMaint ? { color: '#dc2626' } : {}}
                  >
                    {n} – {r?.label}
                    {isConflict ? ' ⚠️ Taken' : ''}
                    {isMaint ? ' 🔧 Under maintenance' : ''}
                  </option>
                )
              })}
            </select>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <div className="flex gap-2">
              ...
              />
              <input
                className="w-24 border border-gray-200 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand/30"
                type="time"
                value={ciTime}
        ...
        <div className="border border-gray-100 rounded-xl p-3 bg-gray-50">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              {
                label: `Extra head (+${peso(addons?.extraHead || 0)}/night)`,
                val: extraHead,
                set: setExtraHead,
              },
              {
                label: `Extra bed (+${peso(addons?.extraBed || 0)}/night)`,
                val: extraBed,
                set: setExtraBed,
              },
              {
                label: `Breakfast (+${peso(addons?.breakfast || 0)}/night)`,
                val: breakfast,
                set: setBreakfast,
              },
            ].map(({ label, val, set }) => (
        ...
        {nights > 0 && (
          <div className="bg-brand/5 border border-brand/20 rounded-xl p-3 text-sm space-y-1">
            <div className="flex justify-between text-gray-600">
              <span>Gross ({nights} nights)</span>
              <span className="font-mono">{peso(gross)}</span>
            </div>
            {discType !== 'none' && discValue > 0 && (
              <div className="flex justify-between text-red-600">
                <span>Discount</span>
                <span className="font-mono">−{peso(gross - net)}</span>
              </div>
            )}
            <div className="flex justify-between font-bold text-brand border-t border-brand/20 pt-1 mt-1">
              <span>Amount due</span>
              <span className="font-mono">{peso(net)}</span>
            </div>
          </div>
        )}
        ...
        {conflict && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">
            🚫 Room {room} is already booked by <strong>{conflict.guest}</strong> (
