...
function bookingIncome(b, rooms, prices, addons, from, to) {
  ...
  const addonRate =
    (b.extra_head || 0) * (addons?.extraHead || 0) +
...
function getPeriodRange(view, offset) {
  ...
  if (view === 'day') {
    ...
    toLabel.setDate(toLabel.getDate() - 1)
...
function buildBars(view, from, to, bookings, rooms, prices, addons) {
  ...
  if (view === 'day') {
    // Hours 0–23
    for (let h = 0; h < 24; h++) {
      const hFrom = new Date(from)
      hFrom.setHours(h)
      const hTo = new Date(from)
      hTo.setHours(h + 1)
      bars.push({ label: `${h}h`, value: 0 })
    }
    // simplify: day total only
    const total = bookings.reduce(
      (s, b) => s + bookingIncome(b, rooms, prices, addons, from, to),
      0,
    )
...
export default function AnalyticsPage() {
  ...
  const bars = useMemo(
    () => buildBars(view, from, to, bookings, rooms, prices, addons),
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4">
          ...
          <div className="text-2xl font-extrabold text-gray-900">
            {
              bookings.filter((b) => parseDate(b.checkin) < to && parseDate(b.checkout) > from)
                .length
            }
          </div>
        </div>
        <div className="bg-white border border-gray-100 rounded-xl p-4">
          ...
          {bySource[0] && (
            <div className="text-xs text-gray-400 mt-0.5">{peso(bySource[0][1])}</div>
          )}
        </div>
      </div>
          ...
          bySource.map(([src, inc]) => (
            <div key={src} className="flex items-center gap-3 mb-2">
              ...
              <div className="flex-1 bg-gray-100 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${SRC_COLORS[src] || 'bg-gray-400'}`}
                  style={{ width: `${(inc / total) * 100}%` }}
                />
              </div>
              <span className="text-sm font-mono font-semibold text-gray-700 w-24 text-right">
                {peso(inc)}
              </span>
              <span className="text-xs text-gray-400 w-10 text-right">
                {Math.round((inc / total) * 100)}%
              </span>
            </div>
          ))
        )}
