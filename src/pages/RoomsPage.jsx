...
export default function RoomsPage({ hotelId }) {
  ...
  const handleAdd = async () => {
    if (!newNumber.trim()) { toast('Enter a room number'); return }
  ...
  return (
    <div className="p-5">
          ...
          return (
            <div key={num} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm">
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={() => handleRemove(room)} className="text-gray-300 hover:text-red-500 text-base">×</button>
              </div>
              <div className="text-xs text-gray-400 mb-3">{room?.label}</div>
              <div className="flex gap-1.5">
                {['vacant','maintenance'].map(s => (
                  <button key={s} onClick={() => handleStatus(room, s)}
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add Room" maxWidth="max-w-sm"
        footer={<>
          <button onClick={() => setAddOpen(false)} className="px-4 py-2 text-sm text-gray-500">Cancel</button>
          <button onClick={handleAdd} className="px-4 py-2 text-sm bg-brand text-white rounded-lg font-semibold">Add Room</button>
        </>}
      >
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Room number</label>
